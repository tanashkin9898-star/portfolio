...
const PROJECTS = {
  live: {
    ...
    what: 'Streamers broadcast to the community, and any viewer can request a Live Match — a short one-on-one video date inside the stream. At the end, both sides like or pass. The aim was to give viewers a reason to stay past the first minute.',
    ...
    impactLine: 'Since launch, streams run 3.4× longer and daily messages tripled.',
    features: [
      ...
      { t: 'The match moves to Messages', d: 'After the stream, the match carries over into a private chat, so the conversation continues outside the broadcast.', img: 'assets/work/live-chat.gif' },
    ],
  ...
  social: {
    ...
    lede: 'Mark is an AR social platform where people pin photos, video, and audio to real-world places. I led UX/UI from concept to MVP.',
    ...
    glance: {
      ...
      result: 'Shipped the MVP covering the scan, place, and view loop — with use cases spanning tourism, education, and AR art events.',
    },
    ...
    peakP: 'No content means no discovery; no discovery means no creators. The whole product hinged on making one complex AR interaction simple enough for a first-time user.',
    solutionH: 'It came down to three flows: scan, place, view.',
    ...
    impactLine: 'Shipped as an MVP covering the full scan, place, and view loop.',
    features: [
      { t: 'The world through the camera', d: 'Point your phone at a place and see the Marks people left there, anchored where they were posted. I kept the interface to a minimum so the camera view stays readable.', img: 'assets/work/ar-scan.gif' },
      ...
      { t: 'Placing a Mark, step by step', d: 'Scan a surface, position the content, then publish. AR placement is unfamiliar for most people, so I broke it into three clear steps with feedback at each one rather than asking users to work it out at once.', img: 'assets/work/ar-place.gif' },
    ],
  ...
  sync: {
    ...
    impactLine: 'Frames become tickets, and the design context travels with them.',
    features: [
  ...
  system: {
    title: 'Design System', category: 'Marketplace platform · Web', kind: 'desktop',
  ...
  gust: {
    ...
    what: 'aiGust connects to the accounting software firms already use (Exact, Yuki) and pulls every client’s books into one hub. From there it generates always-up-to-date reports, benchmarks each client against anonymised peers, and uses AI to flag anomalies worth a conversation — so the accountant has the context before a client meeting, not just the raw figures.',
